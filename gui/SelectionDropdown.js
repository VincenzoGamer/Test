...
global.export.SelectionDropdown = class {
  ...
  Draw(x, y, MouseX, MouseY) {
    ...
    let w = (this.panel.width - 25) / 3  // Divide width by 3 instead of 2
    let ox = 0
    let oy = 11 + this.font.getHeight(this.name)
    this.buttons.forEach((btn, i) => {
      btn.Draw(this.x + 10 + ox, this.y + oy, w, 10, MouseX, MouseY)

      if (i % 3 === 2) {  // Check for every third button
        ox = 0
        oy += 13
      } else {
        ox += w + 2
  ...
  Click() {
    if (this.mouseOver && !this.open) {
      ...
      this.panel.width = 200  // Increased width to accommodate 3 columns
      this.panel.height = 13 + (this.font.getHeight(this.name) + Math.ceil(this.options.length / 3) * 13)  // Divide by 3 instead of 2
