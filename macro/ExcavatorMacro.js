...
let { chat, InventoryUtils, Utils, MouseUtils, TimeHelper } = global.export
...
class ExcavatorMacro {
    constructor() {
        ...
        register("tick", () => {
            ...
            if (this.stuckTimer.hasReached(this.clickDelay * 150 * 50)) {
                if (!this.isGuiNull()) {
                    InventoryUtils.closeInv()
                }
                this.resetState()
                this.state = this.STATES.OPENING
                chat.debugMessage("Reached stuck timer. Attempting to auto-restart.")
            }
            switch (this.state) {
    ...
    handleOpening() {
        ...
        if (!this.usedScrap && !this.hasItem("scrap")) {
            this.stopMacroWithWarning("No scrap found.")
            return
        }
        if (!this.usedScrap && !this.hasItem("armor_stand", false)) {
            this.stopMacroWithWarning("No chisel found.")
            return
        }
        ...
        if (this.openingExcavator) {
            if (this.isGuiNull()) {
                if (Player.lookingAt() instanceof Entity) {
                    ...
                    this.stopMacroWithWarning("Not looking at excavator?")
                    return
                }
            }
        }
    ...
    toggle() {
        ...
        if (this.Enabled) {
            ...
            global.export.FailsafeManager.register(
                ...
                ["Teleport", "Rotation", "Velocity", "Item", "Block"],
              )
