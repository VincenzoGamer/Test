...
global.modules.push(
  new ConfigModuleClass(
    ...
    "Mining",
    [
      new SettingSelector("Gemstone Route", 0, ["Custom - 1", "Custom - 2", "Custom - 3", "Custom - 4", "Custom - 5", "Custom - 6", "Custom - 7", "Custom - 8", "Custom - 9"]),
    ...
    if (this.Enabled) {
      ...
      if (this.renderDisplay) {
        ...
        try {
          ...
          chat.debugMessage("Failed to fetch Gemstone Prices.")
        }
    ...
    try {
      if (routeObject[0].x !== undefined) {
        ...
        for (let i = 0; i < routeObject.length; i++) {
          ...
          if (nextPoint.x === currentPoint.x && nextPoint.y === currentPoint.y && nextPoint.z === currentPoint.z) {
            continue
          }
        ...
        newRoute = tempRoute
        this.sendMacroMessage("Converted your route to Rdbt format.")
      }
    } catch (error) {}
    return newRoute
  }
